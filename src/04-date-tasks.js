...
function timeSpanToString(startDate, endDate) {
  ...
  const hours = (endHours - startHours).toString().padStart(2, 0);
  const startMinutes = startDate.getMinutes();
  const endMinutes = endDate.getMinutes();
  const minutes = (endMinutes - startMinutes).toString().padStart(2, 0);
  const startSeconds = startDate.getSeconds();
  const endSeconds = endDate.getSeconds();
  const seconds = (endSeconds - startSeconds).toString().padStart(2, 0);
...
function angleBetweenClockHands(date) {
  ...
  const hoursNorm = hours >= 12 ? hours - 12 : hours;
  const minutes = newDate.getUTCMinutes();
  const angleHourHand = hoursNorm * 30 + minutes * 0.5;
